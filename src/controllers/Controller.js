class Controller {
  ...
  async atualiza (req, res) {
    const { id } = req.params;
    const dadosAtualiados = req.body;
    try {
      const foiAtualizado = await this.entidadeService.atualizaRegistro(dadosAtualiados, Number(id));
      if (!foiAtualizado) {
        return res.status(400).json( {menssage: 'registro não foi atualizado'} );
      }
